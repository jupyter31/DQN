...
class AbstractDQN(QN, ABC):
    ...
    def __init__(self, env, config, logger=None):
        ...
        self.summary_writer = SummaryWriter(
            self.config["output"]["output_path"], max_queue=1e5
        )
    ...
    def update_target(self) -> None:
        """ Update target will be called periodically to copy Q network to target Q network """
        raise NotImplementedError
    ...
    def process_state(self, state: torch.Tensor) -> torch.Tensor:
        """
        Processing of state
    ...
    def build(self):
        ...
        if "load_path" in self.config["model_training"].keys():
            print(
                "Loading parameters from file:",
                self.config["model_training"]["load_path"],
            )
            load_path = Path(self.config["model_training"]["load_path"])
            assert (
                load_path.is_file()
    ...
    def save(self):
        ...
        Saves session
        """
        torch.save(self.q_network.state_dict(), self.config["output"]["model_output"])
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        total_norm = torch.nn.utils.clip_grad_norm_(
            self.q_network.parameters(), self.config["model_training"]["clip_val"]
        )
